export const SOURCE = [
  {
    summary: "Qual a durabilidade do produto?",
    detail: "O produto tem a validade de um ano.",
  },
  {
    summary: "O Produto mata ratos?",
    detail: "Não. O produto não é recomendado para exterminar ratos.",
  },
  {
    summary: "O produto faz mal para pele?",
    detail:
      " Não. Mais é recomendado evitar o contato com a pele, sempre lavar bem as mãos após o uso.",
  },
  {
    summary: "O produto faz mal para pets ou animais de estimação?",
    detail:
      "Não. O produto é recomendado para o combate de pulgas, piolhos, carrapatos em cães gatos e aves.",
  },
  {
    summary: "Posso borrifar o produto quantas vezes ao dia?",
    detail:
      "É recomendado borrifar uma vez ao dia, o produto tem a sua ação por 16 horas no ambiente.",
  },
  {
    summary: "Como armazenar o produto?",
    detail:
      "Em um ambiente seco e arejado, matendo em um local seguro fora do alcance de crianças.",
  },
  {
    summary: "O produto mancha as roupas?",
    detail: "Não. O produto não mancha as roupas.",
  },
  {
    summary: "O produto danifica móveis ou eletro domesticos?",
    detail:
      "O produto não danifica móveis é indicado para o combate á cupins. Para eletro domesticos evitar o contato com o equipamento.",
  },
];
